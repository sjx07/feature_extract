...
def main(argv=None) -> int:
    ...
    if a.cmd == "library":
        ...
        jid = start(store, ws, f"library:{a.sub}", a.corpus, model, {"kind": a.kind, "version": a.version, "workers": a.workers, "effort": a.effort, "from": "cli"}, 0)
        print(f"job {jid}: {a.sub} {a.kind} on {a.corpus}, log {ws.job_log(jid)}")
        status = run_library(store, ws, Client(store, base_url=a.base_url), jid, a.corpus, a.kind, a.sub, model=model, workers=a.workers, version=a.version, effort=a.effort, echo=lambda line: print("  " + line, flush=True))
