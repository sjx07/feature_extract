...
from fx.ingest.generalize.cards import cards, seed_codebook_id
...
def globals_(store: Store, kind: str) -> list[dict]:
    """The seed tree with each global's members (cards) per corpus; see fx.ingest.loop.tree for the rest."""
...
def open_cards(store: Store, kind: str) -> list[dict]:
    from fx.ingest.loop import open_units
    from fx.ingest.generalize.level import feature_level
