"""Stage 3, the seed library: the per-corpus libraries aligned into global features, the loop of fx.ingest.loop on the
feature level (fx.ingest.generalize.level). This package adds the cards (the units) and the seed codebook, and keeps the kind-only
...
from fx.ingest import loop as E
from fx.ingest.induce.codebook import COLDSTART_MODEL
from fx.core.llm.registry import DEFAULT_MODEL
from fx.core.store import Store
from fx.ingest.generalize.cards import cards, libraries
from fx.ingest.generalize.level import feature_level
from fx.ingest.generalize.seed import globals_, open_cards, regroup_by_aspect, reset, seed_codebook, status
