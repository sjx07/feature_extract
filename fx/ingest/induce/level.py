...
from fx.data.corpus import corpus_domain
from fx.ingest.loop import Level, render_proposals, vectors
from fx.core.store import Store
from fx.ingest.induce import prompts as P
from fx.ingest.induce.codebook import BATCH, MIN_SUPPORT
...
def wording_level(store: Store, codebook: Optional[int] = None, corpus: Optional[str] = None, kind: Optional[str] = None) -> Level:
    ...
    lv = Level(
        ...
        prompt_join=lambda tr, ps, us, qs: P.join_prompt(kind, domain, tr, render_proposals(ps, "R"), us, qs, near=_near_all(store, lv_ref[0], tr, ps)),
...
def _near(store: Store, lv: Level, tr: list[dict], members: list[dict]) -> set:
    ...
    from fx.ingest.loop import node_vectors, vectors
