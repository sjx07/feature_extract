...
async function viewNode(main, id, q) {
  ...
  main.innerHTML = `<div style="font-size:12.5px;color:var(--muted);margin-bottom:8px"><a href="${href('/library', { ...q, kind })}">Library</a> › ${d.group ? esc(d.group.name) + ' › ' : ''}${d.variant_of ? `<a href="${nodeHref(d.variant_of.id)}">${esc(d.variant_of.name)}</a> › ` : ''}${d.global ? 'global feature' : `<span class="corp">${esc(d.corpus)}</span>`}</div>
    <h1>${esc(d.name)}</h1><p class="lede">${esc(d.definition)}</p>
    <div class="muted" style="margin-bottom:14px">${fmt(d.prompts)} of ${fmt(d.selected)} prompts in the slice, ${fmt(d.readings)} readings${d.variant_of ? `, a variant of <a href="${nodeHref(d.variant_of.id)}">${esc(d.variant_of.name)}</a>` : ''}${d.global ? `, ${d.members.length} per-corpus features` : d.global_of ? `, aligned to <a href="${nodeHref(d.global_of)}">${esc(d.global_name)}</a>` : ', not under a global yet'}${d.global ? '' : `, <a href="${href('/feature/' + d.id)}" class="muted">its codebook page</a>`}${filterChips(q, kind, 'features') ? ' ' + filterChips(q, kind, 'features') : ''}</div>
