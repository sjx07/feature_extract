...
async function viewLibrary(main, q) {
  ...
  main.innerHTML = `<div class="split"><div>
    ...
    <p class="muted" style="font-size:12.5px">The tree only grows. A round clusters the open wordings by retrieval, names each candidate cluster (a variant under a feature, a new feature, or a rejection), and assigns the open wordings against the tree again; the judge's flagged members go back to open, barred from the node they left. It ends when no candidate is left or naming yields under three nodes. A wording no other prompt echoes is marked specific and waits. Each step is one line in the job log; a re-run resumes.</p>
