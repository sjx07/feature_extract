...
async function viewLibrary(main, q) {
  ...
  const vtable = vs.length ? `<table class="list"><tr><th>version</th><th>round</th><th>model</th><th class="n">groups</th><th class="n">features</th><th class="n">variants</th><th class="n">rounds</th><th class="n">assigned</th><th class="n">open</th><th class="n">of which specific</th><th class="n">reading coverage</th><th class="n">anchors</th><th class="n">flags</th></tr>
    ${vs.map(v => `<tr><td><a href="${href('/library', { corpus, kind, version: v.version })}">v${v.version}</a>${cb && v.id === cb.id ? ' <span class="muted">shown</span>' : ''}</td><td class="n">${v.round}</td><td>${esc(v.model)}</td><td class="n">${v.groups}</td><td class="n">${v.features}</td><td class="n">${v.variants}</td><td class="n">${v.rounds}</td><td class="n">${fmt(v.assigned)}${v.unassigned ? ` <span class="muted">(+${fmt(v.unassigned)} to do)</span>` : ''}</td><td class="n">${fmt(v.leftover)}</td><td class="n">${fmt(v.specific)}</td><td class="n">${pct(v.reading_coverage)}</td><td class="n">${v.anchor_agreement == null ? '' : pct(v.anchor_agreement)}</td><td class="n">${v.flags}</td></tr>`).join('')}</table>` : '<span class="muted">no codebook yet: run cold start</span>';
  ...
  main.innerHTML = `<div class="split"><div>
    ...
    <form id="lform" class="form" style="grid-template-columns:90px minmax(0,1fr)">
      <label>step</label><select name="step"><option value="round" selected>round loop: cold start if needed, assign, judge, then cluster → name → assign until no candidates</option><option value="coldstart">cold start</option><option value="assign">assign (new and open wordings)</option><option value="judge">judge (read-only)</option><option value="cluster">cluster the open wordings (no calls)</option><option value="name">name the candidate clusters</option></select>
